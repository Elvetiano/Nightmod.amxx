...
public cmdAddBan(id, level, cid)
{
	...
	*/
	...
	new bool:is_ip = bool:(containi(target_IP, ".") != -1);
	...
	// Try to find the player that should be banned
	g_choicePlayerId[id] = 123 //locate_player(id, g_ident)
	
		
	if(g_being_banned[g_choicePlayerId[id]]) {
		if ( get_pcvar_num(pcvar_debug) >= 1 )
			log_amx("[AMXBans Blocking doubleban(g_being_banned)] Player: %s BanLenght: %s Reason: %s", target_name, g_choiceTime[id], g_choiceReason[id])
	...
	if ( get_pcvar_num(pcvar_debug) >= 1 )
		log_amx("[AMXBans cmdBan function 1] Player: %s", target_name)
