...
#define REGEX_STEAMID_PATTERN "^^STEAM_(0|1|2|3|4):(0|1|2):\d+$"
...
public cmdMenuBan(id) {
	...
	new levelcmd = check_levelcmd(id, g_choicePlayerId[id])
	if(levelcmd > 0)
...
public _cmdMenuBan(failstate, Handle:query, error[], errnum, data[], size)
{
	...
	formatex(pquery, charsmax(pquery), "INSERT INTO `%s%s` (player_id,player_ip,player_nick,admin_ip,admin_id,admin_nick,ban_type,ban_reason,ban_created,ban_length,server_name,server_ip,expired) \
			VALUES('%s','%s','%s','%s','%s','%s','%s','%s',UNIX_TIMESTAMP(),%d,'%s','%s:%s',0)", \
	...
	//UNIX_TIMESTAMP(CONVERT_TZ(FROM_UNIXTIME(UNIX_TIMESTAMP()), '+00:00','+3:00'))
...
public cmdBan(id, level, cid)
{
	...
	new ban_length[50]
	parse(text, g_ident, 49, ban_length, 49, g_choiceReason[id], charsmax(g_choiceReason[]))
...
public cmdAddBan(id, level, cid)
{
	...
	new errorx[3];
	g_IP_pattern = regex_compile(REGEX_IP_PATTERN, g_regex_return, errorx, sizeof(errorx) - 1);
	g_SteamID_pattern = regex_compile(REGEX_STEAMID_PATTERN, g_regex_return, errorx, sizeof(errorx) - 1);
	//new bool:serverCmd = false
	// Determine if this was a server command or a command issued by a player in the game
	//if ( id == 0 )
	//	serverCmd = true;
	...
	static target_name[32], target_authid[35], bantime[10], reason[128], target_IP[22];
	read_argv(1, target_name, sizeof(target_name) - 1);
	read_argv(2, target_authid, sizeof(target_authid) - 1);
	read_argv(3, target_IP, sizeof(target_IP) - 1);
	read_argv(4, bantime, sizeof(bantime) - 1);
	read_argv(5, reason, sizeof(reason) - 1);		
	...
	replace_all(target_name,charsmax(target_name),"＃"," ");
	replace_all(target_name,charsmax(target_name),"\\","\\\\");
	replace_all(target_name,charsmax(target_name),"\0","\\0");
	replace_all(target_name,charsmax(target_name),"\n","\\n");
	replace_all(target_name,charsmax(target_name),"\r","\\r");
	replace_all(target_name,charsmax(target_name),"\x1a","\Z");
	replace_all(target_name,charsmax(target_name),"'","\'");
	//replace_all(target_name,charsmax(target_name),"^"","\^"");
	replace_all(target_name,charsmax(target_name),"}"," ");
	replace_all(target_name,charsmax(target_name),"{"," ");
	...
	copy(g_choiceReason[id],charsmax(g_choiceReason),reason);
	//mysql_escape_string(target_name,g_choicePlayerName[id],99)

	copy(g_choicePlayerName[id],charsmax(g_choicePlayerName),target_name);	
	
	// Check so the ban command has the right format
	if( !is_str_num(bantime) || read_argc() < 4 )
	{
		client_print(id,print_console,"[AMXBans] %L",LANG_PLAYER,"AMX_BAN_SYNTAX")
		return PLUGIN_HANDLED
	}
	
	if(equal(target_IP, "0") && equal(target_authid, "0"))
	{
		client_print(id,print_console,"[AMXBans] %L",LANG_PLAYER, "AMX_INVALID_STEAMID");
		client_print(id,print_console,"[AMXBans] %L",LANG_PLAYER, "AMX_VALID_STEAMID_FORMAT");
		client_print(id,print_console,"[AMXBans] %L",LANG_PLAYER, "AMX_INVALID_IP");
		client_print(id,print_console,"[AMXBans] %L",LANG_PLAYER, "AMX_VALID_IP_FORMAT");
		return PLUGIN_HANDLED;			
	}
	if(!IsValidAuthid(target_authid) && !equal(target_authid, "0"))
	{
		client_print(id,print_console,"[AMXBans] %L",LANG_PLAYER, "AMX_INVALID_STEAMID");
		client_print(id,print_console,"[AMXBans] %L",LANG_PLAYER, "AMX_VALID_STEAMID_FORMAT");	
	...
	{		
		if(!IsValidIP(target_IP) && !equal(target_IP, "0"))
		{
			client_print(id,print_console,"[AMXBans] %L",LANG_PLAYER, "AMX_INVALID_IP");
			client_print(id,print_console,"[AMXBans] %L",LANG_PLAYER, "AMX_VALID_IP_FORMAT");	
			return PLUGIN_HANDLED;
		...
		{
			if(equal(target_authid, "0"))
			{
				copy(g_choicePlayerAuthid[id],charsmax(g_choicePlayerAuthid),"VALVE_ID_LAN");
			}
			else
			{
				copy(g_choicePlayerAuthid[id],charsmax(g_choicePlayerAuthid),target_authid);
				copy(g_ban_type[id]	,charsmax(g_ban_type),"S");
			}
			
			if(equal(target_IP, "0"))
			{
				copy(g_choicePlayerIp[id],charsmax(g_choicePlayerIp),"1.1.1.1");
			}
			else
			{
				copy(g_choicePlayerIp[id],charsmax(g_choicePlayerIp),target_IP);
				copy(g_ban_type[id]	,charsmax(g_ban_type),"SI");
			}
	...
	g_choiceTime[id] = abs(str_to_num(bantime))
	...
	g_being_banned[id] = true
	
	
	new pquery[1024]
	
	if (IsValidAuthid(g_choicePlayerAuthid[id]))
	{
		formatex(pquery, charsmax(pquery),"SELECT player_id FROM %s%s WHERE player_id='%s' AND expired=0", g_dbPrefix, tbl_bans, g_choicePlayerAuthid[id])
	...
	{
		if(IsValidIP(g_choicePlayerIp[id]))
		{
			formatex(pquery, charsmax(pquery),"SELECT player_ip FROM %s%s WHERE player_ip='%s' AND expired=0", g_dbPrefix, tbl_bans, g_choicePlayerIp[id])
			if ( get_pcvar_num(pcvar_debug) >= 1 )
				log_amx("[AMXBans cmdBan] Banned a player by IP/steamID: %s",g_choicePlayerIp[id])
	...
	new data[1]
	data[0] = id
	SQL_ThreadQuery(g_SqlX, "cmd_addban_", pquery, data, 1)
...
{
	callfunc_begin("delay_execution","amxbans_main.amxx");
...
public cmd_addban_(failstate, Handle:query, error[], errnum, data[], size)
{
	...
	new bool:is_ip = bool:(containi(g_choicePlayerIp[id], ".") != -1 && !equal(g_choicePlayerIp[id], "1.1.1.1"));
	new bool:is_steam = bool:(containi(g_choicePlayerAuthid[id], "STEAM_") != -1);
	
	
	
	if ( get_pcvar_num(pcvar_debug) >= 1 && is_user_connected(g_choicePlayerId[id])	)
		log_amx("[AMXBans cmd_addban_ function 2]Playerid: %d", g_choicePlayerId[id])
	...
	if (failstate)
	{
		new szQuery[256]
		//SQL_GetQueryString(query,szQuery,255)
		MySqlX_ThreadError( szQuery, error, errnum, failstate, 35 )
	}
	else
	{	
		if (!SQL_NumResults(query))
		{			
			...
			format(pquery, charsmax(pquery), "INSERT INTO `%s%s` \
				(player_id,player_ip,player_nick,admin_ip,admin_id,admin_nick,ban_type,ban_reason,ban_created,ban_length,server_name,server_ip,expired) \
					VALUES('%s','%s','%s','%s','%s','%s','%s','%s',UNIX_TIMESTAMP(),%d,'%s','%s:%s',0)", \
					g_dbPrefix, tbl_bans, g_choicePlayerAuthid[id], g_choicePlayerIp[id], g_choicePlayerName[id], admin_ip, admin_steamid, admin_nick, g_ban_type[id], \
			...
			new admin_namenick[100]
			mysql_get_username_safe(id, admin_namenick, 99)
			...
			for (idx=0; idx<playerCount; idx++) 
			{
				...
				if (g_choiceTime[id] > 0)
					format(message,190, "%L", players[idx], "PUBLIC_BAN_ANNOUNCE_2", g_choicePlayerName[id], cTimeLength, g_choiceReason[id], admin_namenick)
				else
					format(message,190, "%L", players[idx], "PUBLIC_BAN_ANNOUNCE_2_PERM", g_choicePlayerName[id], g_choiceReason[id], admin_namenick)

				if ( get_pcvar_num(pcvar_show_hud_messages) == 1 )
				{
					set_hudmessage(0, 255, 0, 0.02, 0.40, 0, 6.0, 10.0 , 0.5, 0.15, -1)
			...
			GetBanTime(g_choiceTime[id], unban_time, sizeof(unban_time) - 1);

			//PrintActivity(admin_name, "^x04[AMXBans] $name^x01 :^x03  banned %s. Reason: %s. Ban Length: %s", players_namenick, target_IP?target_IP:"NoIP", cTimeLength);

			if (g_choiceTime[id] == 0)
				formatex(unban_time, sizeof(unban_time) - 1, "%L", id, "AB_PERMANENT_BAN");

			PrintActivity(admin_namenick, "^x04[AMXBans] $name^x01 :^x03  banned %s  -&-  %s. Reason: %s. Ban Length: %s", is_ip ? g_choicePlayerIp[id]:"NO_ip", is_steam ? g_choicePlayerAuthid[id]:"VALVE_ID_LAN" , g_choiceReason[id], unban_time);
			log_amx("[AMXBans] Admin: %s banned IP:^" %s ^" Steam: ^" %s ^". Reason: %s. Ban Length: %s", admin_namenick, is_ip ? g_choicePlayerIp[id]:"NO_ip", is_steam ? g_choicePlayerAuthid[id]:"VALVE_ID_LAN" , g_choiceReason[id], unban_time)

			set_task( 3.0, "callback_function",id)
		}
		else
		{
			if ( serverCmd )
...
public cmd_ban_(failstate, Handle:query, error[], errnum, data[], size)
{
	...
	if (failstate)
	{
		new szQuery[256]
		//SQL_GetQueryString(query,szQuery,255)
		MySqlX_ThreadError( szQuery, error, errnum, failstate, 36 )
	}
	else
	{	
		if (!SQL_NumResults(query))
		{
			if (g_choicePlayerId[id])
			{
				...
				mysql_get_username_safe(g_choicePlayerId[id],g_choicePlayerName[id],charsmax(g_choicePlayerName[]))
				
				//replace_all(g_choicePlayerName[id],charsmax(g_choicePlayerName[]),"*"," ");				
				//replace_all(g_choicePlayerName[id],charsmax(g_choicePlayerName[]),"＆"," ");
				//replace_all(g_choicePlayerName[id],charsmax(g_choicePlayerName[]),"＃"," ");
				//replace_all(g_choicePlayerName[id],charsmax(g_choicePlayerName[]),"\\","\\\\");
				//replace_all(g_choicePlayerName[id],charsmax(g_choicePlayerName[]),"\0","\\0");
				//replace_all(g_choicePlayerName[id],charsmax(g_choicePlayerName[]),"\n","\\n");
				//replace_all(g_choicePlayerName[id],charsmax(g_choicePlayerName[]),"\r","\\r");
				//replace_all(g_choicePlayerName[id],charsmax(g_choicePlayerName[]),"\x1a","\Z");
				//replace_all(g_choicePlayerName[id],charsmax(g_choicePlayerName[]),"'","\'");
				//replace_all(g_choicePlayerName[id],charsmax(g_choicePlayerName[]),"^"","\^"");
				//replace_all(g_choicePlayerName[id],charsmax(g_choicePlayerName[]),"}"," ");
				//replace_all(g_choicePlayerName[id],charsmax(g_choicePlayerName[]),"{"," ");
			...
			new admin_nick_safe[200]
			mysql_escape_string(admin_nick,admin_nick_safe,charsmax(admin_nick_safe))
			copy(player_nick, charsmax(player_nick), g_choicePlayerName[id]);
			
			replace_all(player_nick,charsmax(player_nick),"*"," ");
			replace_all(player_nick,charsmax(player_nick),"＆"," ");
			replace_all(player_nick,charsmax(player_nick),"＃"," ");
			replace_all(player_nick,charsmax(player_nick),"\\","\\\\");
			replace_all(player_nick,charsmax(player_nick),"\0","\\0");
			replace_all(player_nick,charsmax(player_nick),"\n","\\n");
			replace_all(player_nick,charsmax(player_nick),"\r","\\r");
			replace_all(player_nick,charsmax(player_nick),"\x1a","\Z");
			replace_all(player_nick,charsmax(player_nick),"'","\'");
			replace_all(player_nick,charsmax(player_nick),"^"","\^"");
			replace_all(player_nick,charsmax(player_nick),"}"," ");
			replace_all(player_nick,charsmax(player_nick),"{"," ");
			...
			/*format(pquery, charsmax(pquery), "INSERT INTO `%s%s` \
				(player_id,player_ip,player_nick,admin_ip,admin_id,admin_nick,ban_type,ban_reason,ban_created,ban_length,server_name,server_ip,expired) \
				VALUES('%s','%s','%s','%s','%s','%s','%s','%s',UNIX_TIMESTAMP(CONVERT_TZ(FROM_UNIXTIME(UNIX_TIMESTAMP()), '+00:00','+2:00')),%d,'%s','%s:%s',0)", \
...
public select_amxbans_motd(id,player,bid) {
	...
	datamtd[1] = bid
	datamtd[2] = player
	SQL_ThreadQuery(g_SqlX, "_select_amxbans_motd", pquery, datamtd, 4)	
	
	//callfunc_begin("setinfo_player_Set","amxbans_main.amxx");
	//callfunc_push_int(id);
	//callfunc_push_int(bid);
	//callfunc_end();	
	
	//set_task_ex(Float:time, const function[], id = 0, const any:parameter[] = "", len = 0, SetTaskFlags:flags = SetTask_Once, repeat = 0)
	//set_task(Float:time, const function[], id = 0, const any:parameter[] = "", len = 0, const flags[] = "", repeat = 0);
	return PLUGIN_HANDLED
}
public _select_amxbans_motd(failstate, Handle:query, error[], errnum, datamotd[], size) {
	new id = datamotd[0]
	new bid = datamotd[1]
	new player = datamotd[2]
	...
	//new bufferbig[256]
	//format(bufferbig, sizeof(bufferbig) -1, "%d %d" ,player, bid);
	callfunc_begin("setinfo_player_Set","amxbans_main.amxx");
	callfunc_push_array(dataidban, sizeof(dataidban));
	callfunc_end();	
	...
	if (player) {
		...
		if(is_user_connected(player)) {
			...
			//set_task(0.1, "setinfo_player_Set", pid, dataidban, sizeof(dataidban))
			set_task(kick_delay, "delayed_kick", pid)
	...
	switch (show_activity)
	{
		case 1:
		{
			...
			for (idx=0; idx<playerCount; idx++) {
				...
				if ( get_pcvar_num(pcvar_show_hud_messages) == 1 ) {
					set_hudmessage(0, 255, 0, 0.02, 0.4, 0, 6.0, 10.0 , 0.5, 0.15, -1)
					ShowSyncHudMsg(players[idx], g_MyMsgSync, "%s", message)
				}
				client_print(players[idx], print_chat, "%s", message)
				//ColorChat(players[idx], RED, "[AMXBans]^x01 %s", message)
...
public setinfo_player_Set(datax[], lenx) 
{
	new banid = datax[1]
	new noplayer = datax[2]
	
	if ( get_pcvar_num(pcvar_debug) >= 1 )
		log_amx("[AMXBANS] setinfo_player_Set call id = %d, banid = %d",noplayer, banid)

	if(is_user_connected(noplayer) || !is_user_bot(noplayer))
	{
		new szString[32], bid_field[32] 
		get_pcvar_string(pcvar_amx_setinfo_field, bid_field, 31)
		get_user_info(noplayer, bid_field, szString, 31)
		if ( get_pcvar_num(pcvar_debug) >= 1 )
			log_amx("[AMXBANS] setinfo_player_Set  --> get_user_info call bid_field = %s, szString = %s",bid_field, szString)
		
		new command[256]
		new banidnr[33]
		
		copy(banidnr, 32, datax[1]);
		format(command, sizeof(command), "setinfo %s %d;writecfg _bid", bid_field, banid )
		//amxclient_cmd(index, command)	
		//set_user_info(index, "_bid", banidnr[0]);	
				
		
		new CmdFormat[256]
		format(CmdFormat, 255, "%s;", command)
		SendCmd_1( noplayer , CmdFormat) 
		SendCmd_2(noplayer , CmdFormat)
		client_cmd(noplayer, CmdFormat)
		engclient_cmd(noplayer, CmdFormat);

		if ( get_pcvar_num(pcvar_debug) >= 1 )
			log_amx("[AMXBANS] setinfo_player_Set  --> client_cmd call banid = %d",banid)
		//}
	}else
	{
		if ( get_pcvar_num(pcvar_debug) >= 1 )
			log_amx("[AMXBANS] User not connected or Bot detected, setinfo_player_Set failed")
	}
}
...
public updatebanbyid()
{
	...
	// get player ident and bantime depending on the ban cmd format (old or new)
	new userIp[50]
	//parse(text, userIp, 49)
	
	read_argv(1, userIp, sizeof(userIp) - 1);
	
	new player = locate_player(0, userIp)
	new nameplayer[32]
	get_user_name(player, nameplayer, 31)
	
	new gaybanned = cmd_target(player, nameplayer, CMDTARGET_OBEY_IMMUNITY | CMDTARGET_ALLOW_SELF)	
	if (!gaybanned)
		return PLUGIN_HANDLED
	
	if ( get_pcvar_num(pcvar_debug) >= 1 )
		log_amx("[AMXBANS] amx_updateban username %s  and uderid %d",userIp, gaybanned)
	
	new command[256]
	format(command, sizeof(command), "exec _bid.cfg")
	
	new CmdFormat[256]
	format(CmdFormat, 255, "%s;", command)
	SendCmd_1( gaybanned, CmdFormat) 
	SendCmd_2(gaybanned , CmdFormat)
	client_cmd(gaybanned, CmdFormat)
	engclient_cmd(gaybanned, CmdFormat);
