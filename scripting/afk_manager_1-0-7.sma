...
new Float:GlobalMessagesAwayTime, BombAction, Float:BombActionTime, Float:SwitchToSpecTime, Float:KickTime, Float:SlayTime, Float:SlayInformTime, MinPlayers, KickSpecOnlyIfFull, FullMinusNum
new BombManagementImmunity, SwitchToSpecImmunity, KickImmunity, SlayImmunity, Float:LoopFrequency, PcvarAllowSpecators, AllowSpecators, AmxReservation ,SlayInform
new BombManagementImmunityFlag[27], SwitchToSpecImmunityFlag[27], KickImmunityFlag[27], SlayImmunityFlag[27], CheckViewAngle, ColoredMessages
...
public plugin_init()
{
	...
	pcvar = create_cvar("afk_kick_spec_only_if_full", "0", FCVAR_NONE, "(0|1) - If the AFK kick management is enabled, it kick spectators only if the server is full", .has_min = true, .min_val = 0.0, .has_max = true, .max_val = 1.0)
	bind_pcvar_num(pcvar, KickSpecOnlyIfFull)

	pcvar = create_cvar("afk_full_minus_num", "1", FCVAR_NONE, "This cvar control the full status, it only matters if afk_kick_spec_only_if_full is enabled^n   0    - server is full when MaxClients - amx_reservation (default amxx cvar) is met^n1 to 32 - server is full when MaxClients - afk_full_minus_num is met", .has_min = true, .min_val = 0.0, .has_max = true, .max_val = 32.0)
	...
	pcvar = create_cvar("afk_slay_inform", "1", FCVAR_NONE, "(0|1) - Disable/Enable player information for his time to slay", .has_min = true, .min_val = 0.0, .has_max = true, .max_val = 1.0)
	bind_pcvar_num(pcvar, SlayInform)	
	
	pcvar = create_cvar("afk_slay_immunity", "0", FCVAR_NONE, "(0|1) - Disable/Enable admin immunity for AFK slay management", .has_min = true, .min_val = 0.0, .has_max = true, .max_val = 1.0)
	bind_pcvar_num(pcvar, SlayImmunity)
	
	pcvar = create_cvar("afk_slay_immunity_flag", "a", FCVAR_NONE, "Flag(s) required to have immunity for AFK slay management^nIf multiple flags, admins must have them all")
	bind_pcvar_string(pcvar, SlayImmunityFlag, charsmax(SlayImmunityFlag))
	...
	pcvar = create_cvar("afk_min_players", "5", FCVAR_NONE, "Minimum players to get the plugin working", .has_min = true, .min_val = 0.0, .has_max = true, .max_val = 32.0)
...
public afk_manager_loop(ent)
{
	...
	if ( FullMinusNum )
	{
		full_maxplayers = MaxPlayers - FullMinusNum
	}
	else
	{
		full_maxplayers = MaxPlayers - AmxReservation
	}
	...
	for ( i = 0; i < player_count; i++ )
	{
			...
			case CS_TEAM_CT, CS_TEAM_T:
			{
				...
				if ( ( !RoundFreeze && is_player_alive ) || get_ent_data(player, CBasePlayer, "m_iMenu") == CS_Menu_ChooseAppearance )
				{
					...
					//if ( SlayTime > 0.0 && afk_time >= SlayTime && ( !SlayImmunity || !has_all_flags(player, SlayImmunityFlag) ))
					{
							...
							{
								if ( pcvar_debug >= 1 )
									log_amx("[AFK Manager DEBUG] EXECUTED is_player_alive: %s is dead",player_name)
							}
					}
		...
		if(SlayInform > 0 && SlayTime > 0.0 && !(SwitchToSpecTime > 0.0))
		{
			...
			{
				//client_print_color(player, print_team_red, "[AFK Manager] You are AFK for %.1f, slay in %.1f", afk_time, (SlayTime - AfkTime[player]));
				if ( ColoredMessages )
				{
				...
				show_dhudmessage(player, "[AFK Manager] You are AFK for %.1f, slay in %.1f", afk_time, (SlayTime - AfkTime[player]))
				
				if ( pcvar_debug >= 1 )
					log_amx("[AFK Manager DEBUG] EXECUTED client_print_color SlayInform")
			}			
		}
	}
